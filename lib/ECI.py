...
class calculation:
    def __init__( self, calc ):
        for key in calc:
            setatr( self, key, calc[key] )
#----END of calculation class------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------   


#----START of electronic_state class------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------   
class electronic_state:
    def __init__( self, state ):
        for key in state:
            setatr( self, key, state[key] )  # Z, M, MS, N
        self.MSx2 = int(2.*self.MS)
        self.S = float(self.M-1)/2.
#----END of electronic_state class------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------   
...
class fragment:
    def __init__( self, **kwargs ):
        ...
        # Build mole object with correct geom and basis. 
        # Charge and spin can be anything that will not raise an error in build (the simpliest option is then to put it to refstate values)
        self.label = flabel
        self.mol = pyscf.gto.Mole() 
        self.mol.atom = geom
        self.mol.unit = 'AU'
        self.mol.basis = basis
        self.mol.charge = refcharge
        #self.mol.spin = refstate['M'] - 1
        self.mol.build()

        self.index = index

        # Generate electronic_state instances
        self.states = [] 
        for c, nstates in fdict['site_states'].items():
            aufbau = fdict['aufbau_site_states'][c]
            for m, n in enumerate(nstates):
                for i in range(n):
                    for msx2 in range(-m,m+1,2):
                        self.states.append( electronic_state( Z=c, M=m+1, MSx2=msx2, N=i+1 ) )  # This is the moment in which site states get their pointers
                        if aufbau["M"] == m+1 and  aufbau["N"] == i+1:
                            self.aufbaustates.append( self.states[-1] )

        # Convert input from SHARC-ECI to internal ECI format
        for c, nstates in fdict['site_states'].items():
            states = [ s for s in self.states if s.Z == c ]
            for i, s1 in enumerate( states ):
                for j, s2 in enumerate( states ):
                    # Add Q
                    self.Q = Q
                    # Add H
                    self.H[(s1,s2)] = H[c][i,j]
                    # Add density matrices
                    for spin in ['tot', 'aa', 'ab', 'ba', 'bb']:
                        self.rho[(s1,s2,spin)] = rho[c].get( (s1.M, s1.MSx2, s1.N, s2.M, s2.MSx2, s2.N, spin), None )
                    # Add optional stuff
                    if 'mu' in kwargs:
                        # Add mu
                        self.mu[(s1,s2)] = mu[:,i,j]
                    if 'dH' in kwargs:
                        # Add dH
                        self.dH[(s1,s2)] = dH[:,:,i,j]
        # Add Dyson orbital
        for spin in ['a', 'b']:
            try:
                self.phi[(s1,s2,spin)] = phi[(s1.Z,s2.Z)][(s1.M, s1.MSx2, s1.N, s2.M, s2.MSx2, s2.N, spin)]
            except:
                pass
...
class excitonic_slater_determinant:
    def __init__( self, sites, site_states ):
        ...
        self.MS = sum( [ self.site_states[site].MS for site in sites ] )
        self.MSx2 = int( 2*self.MS )
        self.index = None
...
class ECI:
    def __init__(self, calc ):
        # Input
        self.job = calculation( calc )
        self.sites = []
        self.ECIbasis = {}
        self.active_pairs = { 'J': { (0,0): [], (0,1): [], (0,2): [], (1,0): [], (1,1): [] }, 'K': { (0,0): [], (0,1): [], (0,2): [], (1,0): [], (1,1): [] } }
    ...
    #----START of ECI.load_fragment------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------   
    def load_fragment( self, **kwargs ):
        self.sites.append( fragment( **kwargs ) )
    #----END of ECI.load_fragment------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------   
