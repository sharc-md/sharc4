...
class ECI:
    ...
    def configure(self):
        ...
        for rank, value in job.eci_level.items():
            if rank == 0:
                ...
                eci_level[rank] = []
                for subset in value:
                    newsubset = set([ sites[label] for label in subset])
        ...
        for rank, value in job.ct_level.items():
            if rank == 0:
                ...
                ct_level[rank] = []
                for subset in value:
                    newsubset = [ sites[label] for label in subset]
                    ct_level[rank].append(newsubset)
    ...
    def get_aufbaus(self):
        ...
        ESDs = [ excitonic_slater_determinant(sites=sites, site_states=aufbau) for aufbau in aufbaus ]
        ...
        for aufbau in ESDs:
            ...
            plus, minus = 0, 0
            for f in sites:
                deltaZ = aufbau.site_statates[f].Z - f.Z
