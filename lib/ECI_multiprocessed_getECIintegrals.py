...
class ECIlogic:
    ...
    @staticmethod
    def do_00_pair(inpair):
        """
        Process a single relationship pair, performing the integral calculations.
        This function runs in parallel in each worker process.
        """
        global _shared_job, _shared_sites
        job = _shared_job
        sites = _shared_sites

        pair = dill.loads(inpair) 
        # List to collect the results
        results = []

        # Extract pair info and index
        ESD = pair['ESDs'][0]
        index = ESD.index
        ...
        for (f1, f2) in actives:
            ...
            results.append(dill.dumps(('ECI_K', (f1, f2), d1, d2, (index, index))))
...
class ECI:
    ...
    #  def __getstate__(self):
        #  # Return the state of the object, but exclude the unpickleable logfile
        #  state = self.__dict__.copy()
        #  state['log'] = None  # Exclude the logfile attribute from pickling
        #  return state

    #  def __setstate__(self, state):
        #  # Restore the state, and recreate any unpickleable attributes if necessary
        #  self.__dict__.update(state)
        #  self.logfile = None  # Logfile is not restored
    ...
    def configure(self):
        ...
        #  mp.get_context().reducer = dill
        #  mp.set_start_method("spawn")  # Or use "spawn" depending on your OS
    ...
    def extract_subbasis_from_global_indices(mole_basis, atoms, global_indices):
        ...
        for atom in atoms:
            ...
            for local_index, shell in enumerate(shells):
            ...
            if filtered_shells:
                subbasis[atom] = filtered_shells.copy()
        return subbasis
    ...
    def calculate_Hmat(self):
        ...
        t1 = time.time()
        ECI_integrals = self.get_ECI_integrals( job, sites, ECIbasis )
        t2 = time.time()
        self.log.print('       Generating ECI-integrals dictionary took '+str(round(t2-t1,3))+' sec.')
        ...
        if integrals != None:
            ...
            for f, nucs__densities__locs in integrals.items():
                for nucs, densities__locs in nucs__densities__locs.items():
                    ...
                    values = oe.contract('ij,nij->n', Fmat, rhos, optimize=True ) 
                    t2 = time.time()
        ...
        if integrals != None:
            ...
            for (f1,f2), densities1__densities2__locs in integrals.items():
                ...
                if not job.ri:
                    ...
                    t1 = time.time()
                    LP12 = oe.contract('mij,ijp,pq,klq->mkl', rhos1, L1, P, L2, optimize=True)
                    #  LP12 = np.einsum('mij,pij,pkl->mkl', rhos1, L1, L2, optimize=True )
                    t2 = time.time()
                    self.log.print('       First contraction took '+str(round(t2-t1,3))+' sec.')
                    t1 = time.time()
        ...
        if integrals != None:
            ...
            for (f1,f2), densities1__densities2__locs in integrals.items():
                ...
                #  if True:
                    ...
                    aos_1 = [ ao for shell in shells_1 for ao in range(f1.mol.ao_loc[shell], f1.mol.ao_loc[shell + 1] if shell + 1 < len(f1.mol.ao_loc) else f1.mol.nao) ]
                    aos_2 = [ ao for shell in shells_2 for ao in range(f2.mol.ao_loc[shell], f2.mol.ao_loc[shell + 1] if shell + 1 < len(f2.mol.ao_loc) else f2.mol.nao) ]
                    #  aos_1 = list(aos_1)
                    #  aos_2 = list(aos_2)
                    ...
                    if len(atoms_1) == 0 or len(atoms_2) == 0:
                        ...
                        self.log.print('      Number of shells per fragment: '+str(len(shells_1))+' and '+str(len(shells_2)))
                    ...
                    bas1 = self.extract_subbasis_from_global_indices( f1.mol.basis, sym1, shells_1 ) 
                    ...
                    bas2 = self.extract_subbasis_from_global_indices( f2.mol.basis, sym2, shells_2 ) 
                    ...
                    auxmol = merge_moles( f1.mol, f2.mol )
                    #  auxmol = calcmol.copy()
                    auxmol.build(basis=job.Kauxbasis)
                    t1 = time.time()
                    P = auxmol.intor('int2c2e')
                    P = sp.linalg.cholesky(P, lower=True)
                    t2 = time.time()
    ...
    @staticmethod
    def get_ECI_integrals(job, sites, ECIbasis):
        """
        The main method that spawns parallel processes and gathers results.
        """
        @staticmethod
        def initialize_worker(serialized_job, serialized_sites):
            """
            Initialize the worker with shared global variables.
            These will not be copied to each worker process.
            """
            global _shared_job, _shared_sites
            _shared_job = dill.loads(serialized_job)
            _shared_sites = dill.loads(serialized_sites)
        #  job = self.job
        #  sites = self.sites
        #  ECIbasis = self.ECIbasis

        serialized_job = dill.dumps(job)
        serialized_sites = dill.dumps(sites) 
        #  x = pickle.dumps(serialized_job)
        #  y = pickle.dumps(serialized_sites)

        # Set up a pool with a number of worker processes equal to the number of CPUs
        pool = mp.Pool(processes=job.ncpu, initializer=initialize_worker, initargs=(serialized_job, serialized_sites))

        # Result container to aggregate the results
        all_results = []

        # Parallelize over the multiplicities and their relationship[(0,0)] pairs
        for m in job.multiplicities:
            serialized_relationship = [ dill.dumps( r ) for r in ECIbasis[m].relationships[(0,0)] ]
            #  serialized_relationship = dill.dumps(relationship)

            #  Prepare the list of tasks to pass to the pool
            #  tasks = [(pair, m) for pair in relationship[(0, 0)]]

            # Map the tasks in parallel using the pool
            #  print(serialized_relationship)
            results00 = pool.map(ECIlogic().do_00_pair, serialized_relationship)
            print(results00)
            exit()
            pool.close()
            pool.join()

            # Collect all the lists returned by each worker into one large list
            for result in results:
                all_results.extend(result)

        # Now convert the collected results into the final ECI_integrals dictionary
        ECI_integrals = {}
        for (k1, k2, k3, k4, loc) in all_results:
    ...
    #  def get_ECI_integrals(job, sites, ECIbasis):
        ...
        #  def do_00_pair( pair ):
            ...
            #  for f in sites:
                #  nucs = tuple([ g for g in sites if (f,g) in actives or (g,f) in actives ])
                #  d = (ESD.site_states[f], ESD.site_states[f], 'tot')
                #  result.append( ('ECI_V', f, nucs, d, (m,index,index) ) )
            #  # J0-integrals
            #  for (f1,f2) in actives:
                #  d1 = ( ESD.site_states[f1], ESD.site_states[f1], 'tot' ) 
                #  d2 = ( ESD.site_states[f2], ESD.site_states[f2], 'tot' ) 
                #  result.append( ( 'ECI_J', (f1,f2), d1, d2, (m,index,index) ) )
            #  # K0-integrals
            #  actives = job.active_integrals['K'][(0,0)]
            #  for (f1,f2) in actives:
                #  d1 = ( ESD.site_states[f1], ESD.site_states[f1], 'aa' ) 
                #  d2 = ( ESD.site_states[f2], ESD.site_states[f2], 'aa' ) 
                #  result.append( ( 'ECI_K', (f1,f2), d1, d2, (m,index,index)) )
                #  d1 = ( ESD.site_states[f1], ESD.site_states[f1], 'bb' ) 
                #  d2 = ( ESD.site_states[f2], ESD.site_states[f2], 'bb' ) 
                #  result.append( ( 'ECI_K', (f1,f2), d1, d2, (m,index,index)) )
            #  return result
        ...
        #  for m in job.multiplicities:
            ...
            #  with Pool(processes=job.ncpu) as pool:
                #  list00 = pool.map( do_00_pair, relationship[(0,0)] )
