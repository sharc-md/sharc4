...
def mean_std_from_files(mean_file, out_dir, xlist, std_file=None, col_list=None):
    ...
    if with_std:
        std_lines = open(std_file, 'r').readlines()
    ...
    if with_std:
        std_array = numpy.array(std_list, float).transpose()
    ...
    if col_list is None:
        for i in range(numpy.size(mean_array, 0)):
            ...
            if with_std:
                plot_std(mean_array[i].tolist()[:len(xlist)], std_array[i].tolist()[:len(xlist)], xlist[:len(mean_array[i].tolist())])
