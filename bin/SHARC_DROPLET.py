...
import numpy as np
# internal
from constants import n_avogadro, au2a, au2newton
...
class SHARC_DROPLET(SHARC_HYBRID):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.QMin.template.update({
            "child": None, 
            "droplet_potentials": None
            })
        self.QMin.template.types.update({
            "child": dict, 
            "droplet_potentials": list
            })
    ...
    def get_features(self, KEYSTROKES: TextIOWrapper | None = None) -> set:
        ...
        if (not self._read_template):
            ...
            #     "Please specify the path to your DROPLET.template file", str, KEYSTROKES=KEYSTROKES, default="DROPLET.template"
            # )
            
            # self.read_template(self.template_file)
            while True:
                self.child_interfacename = question("Please specify the name of the child interface", str, KEYSTROKES=KEYSTROKES, default=None, autocomplete=False)
                try:
    ...
    def get_infos(self, INFOS, KEYSTROKES: TextIOWrapper | None = None) -> dict:
        ...
        if question("Do you have an DROPLET.resources file?", bool, KEYSTROKES=KEYSTROKES, autocomplete=False, default=False):
            self.resources_file = question("Specify path to DROPLET.resources", str, KEYSTROKES=KEYSTROKES, autocomplete=True, 
                                            default='DROPLET.resources')

        # --- Setup droplet potentials ---
        while True:
            n_droplets = question("How many droplet/tether potentials do you want to define?", int, KEYSTROKES=KEYSTROKES, default=[1])[0]
            if 0 <= n_droplets:
                break
            else:
        ...
        for i in range(n_droplets):
            ...
            # name = question("Specify a name for this droplet potential", str, default=f"droplet{i+1}")
            name = f"droplet{i+1}"

            if question("Do you want to calculate the parameters from density/system size?", bool, KEYSTROKES=KEYSTROKES, default=False):
                density = question("Specify the density of your solvent [g/mL] (default: water at 298K)", float, KEYSTROKES=KEYSTROKES, default=[0.9974])[0]
                press_pascal = (
                    question("Specify the desired pressure at the surface of the droplet in bar", float, KEYSTROKES=KEYSTROKES, default=[1])[0] * 100_000
                )
                wokness = question(
                    "On a scale from 1 (harmonic) to 0 (hard wall), how fast should the potential increase?",
                    float, KEYSTROKES=KEYSTROKES, default=[0.2]
                )[0]
                molar_mass = question("Specify the molar mass of your solvent [g/mol] (default: water)", float, KEYSTROKES=KEYSTROKES, default=[18.01528])[0]
            ...
            # origin
            # if question("Do you want the origin to be the COM of atoms?", bool, default=False):
            #     origin = "COM"
            # else:
            while True:
                origin = question("Specify the origin in Å", float, KEYSTROKES=KEYSTROKES, default=[0., 0., 0.])
                if len(origin) == 3:
                    break
                else:
    ...
    def read_template(self, template_file="DROPLET.template", kw_whitelist=None):
            ...
            if "child" in tmpl_dict:
                ...
                for k, expected_type in self._child_template.items():
                    if k not in child_config or not isinstance(child_config[k], expected_type):
                        ...
                        self.log.error(
                            f"Child '{name}' is missing key '{k}' "
                            f"or value is not of type {expected_type.__name__}"
                        )
                        raise ValueError
            else:
        ...
        for droplet in tmpl_dict["droplet_potentials"]:
            ...
            if atoms is None:
                # raise ValueError(f"Droplet '{name}': atoms is required")
                atoms = "all"
            ...
            if origin is None:
                # raise ValueError(f"Droplet '{name}': origin is required")
                origin = [0.,0.,0.]
            if isinstance(origin, str):
                ...
                and len(origin) == 3
                and all(isinstance(x, (int, float)) for x in origin)
            ):
