...
''' % (['', '(Only numerical gradients)'][INFOS['freq']]))
  ...
  ANO:       ANO-S-vdzp, ANO-L, ANO-RCC                   ''' % (['', '(Not available)'][ctype == 3]))
    ...
    INFOS['basis'] = basis
    INFOS['cholesky'] = question('Use Cholesky decomposition?', bool, True)

    # douglas kroll
    # dk = question('Douglas-Kroll scalar-relativistic integrals?', bool, False)
    INFOS['DK'] = False
...
''' % (cwd,
    ...
    for imult, nstate in enumerate(INFOS['cas.nstates']):
        if nstate == 0:
            continue
        mult = imult + 1
        string += 'cp $HomeDir/MOLCAS.input $HomeDir/%sOrbitals.RasOrb $WorkDir\n' % (IToMult[mult])
    string += 'cd $WorkDir'
