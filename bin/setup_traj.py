        ...
        while True:
            ...
            for i, nst in enumerate(actstates):
                if not 0 <= nst <= INFOS['states'][i]:
                    log.info('Number of active states of multiplicity %i must be between 0 and the number of states of this multiplicity (%i)!' % (i + 1, INFOS['states'][i]))
                    valid = False
            if not valid:
                continue
            break
...
#      log.info('\nPlease enter all atom indices (start counting at 1) of the atoms which should be masked. \nRemember that you can also enter ranges (e.g., "-1~-3  5  11~21").')
    ...
    log.info('''In order to speed up calculations, SHARC is able to select which gradients and NAC vectors it has to calculate at a certain timestep. The selection is based on the energy difference between the state under consideration and the classical occupied state.
''')
    if INFOS['surf'] == 'diagonal':
        if INFOS['soc']:
    ...
    if INFOS['laser']:
        log.info('''Please specify the file containing the complete laser field. The timestep in the file and the length of the file must fit to the simulation time, time step and number of substeps given above.
