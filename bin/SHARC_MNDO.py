...
AUTHORS = ""
VERSION = ""
VERSIONDATE = datetime.datetime(2023, 10, 16)
NAME = "MNDO"
DESCRIPTION = ""
...
class SHARC_MNDO(SHARC_ABINITIO):
    ...
    def execute_from_qmin(self, workdir: str, qmin: QMin) -> tuple[int, datetime.timedelta]:
        ...
        if self.QMin.molecule["point_charges"]:
            ...
            writefile(os.path.join(workdir, "MNDO.pc"), pc_str)

        # Copy wf files
        jobid = qmin.control["jobid"]
        if qmin.control["master"] and jobid in qmin.control["initorbs"]:
            self.log.debug("Copy MNDO.gbw to work directory")
            shutil.copy(qmin.control["initorbs"][jobid], os.path.join(workdir, "MNDO.gbw"))
        elif qmin.control["gradonly"]:
            self.log.debug(f"Copy MNDO.gbw from master_{jobid}")
            shutil.copy(
                os.path.join(qmin.resources["scratchdir"], f"master_{jobid}", "MNDO.gbw"), os.path.join(workdir, "MNDO.gbw")
            )

        # Setup MNDO

        starttime = datetime.datetime.now()
        exec_str = f"{os.path.join(qmin.resources['mndodir'],'mndo')} MNDO.inp"
        exit_code = self.run_program(workdir, exec_str, os.path.join(workdir, "MNDO.log"), os.path.join(workdir, "MNDO.err"))
    ...
    #what is  meant by AO-matrix? it seems that this calculates the overlap and takes information from there!!
    def _get_ao_matrix(self, workdir: str) -> None:
        """
        Call mndo_fragovl and extract ao matrix
        """
        orca_gbw = os.path.join(workdir, "mndo.gbw")
        self.log.debug(f"Extracting AO matrix from {mndo_gbw} for ion request")

        # TODO: REFACTOR!

        # run mndo_fragovl
        string = f"mndo_fragovl {mndo_gbw} {mndo_gbw}"
        try:
    ...
    #Again overlap is calculated!!
    def _get_mos(self, gbw_file: str, jobid: int) -> str:
        """
        Extract MO coefficients from MNDO.out file
        ...
        # run mndo_fragovl
        string = "mndo_fragovl %s %s" % (gbw_file, gbw_file)
        try:
            proc = sp.Popen(string, shell=True, stdout=sp.PIPE, stderr=sp.PIPE)
    ...
    def getQMout(self) -> None:
        ...
        for job in self.QMin.control["joblist"]:
            with open(os.path.join(self.QMin.resources["scratchdir"], f"master_{job}/MNDO.log"), "r", encoding="utf-8") as file:
                log_file = file.read()
        ...
        if self.QMin.requests["grad"]:
            for grad in self.QMin.maps["gradmap"]:
                ...
                if ground_state:
                    gradients = self._get_grad(os.path.join(self.QMin.resources["scratchdir"], job_path, "MNDO.engrad"), True)
                else:
                    gradients = self._get_grad(
                        os.path.join(
                            self.QMin.resources["scratchdir"],
                            job_path,
                            f"MNDO.engrad.{'singlet' if grad[0] == grad_mult[0] else IToMult[grad[0]].lower()}.root{grad[1] - (grad[0] == grad_mult[0])}.grad.tmp",
                        )
                    )
    ...
    def _get_dipole_moment(self, output: str, ground_state: bool) -> np.ndarray:
        """
        Extract dipole moment from MNDO outfile
        output:     Content of outfile as string
        """
        
        find_dipole = re.findall(r"SUM             [-+]?(?:\d*\.*\d+)     [-+]?(?:\d*\.*\d+)     [-+]?(?:\d*\.*\d+)     [-+]?(?:\d*\.*\d+)", output)
        if not find_dipole:
            self.log.error("Cannot find dipole moment in MNDO outfile!")
            raise ValueError()
    ...
    @staticmethod
    def get_MNDO_version(path: str) -> tuple[int, ...]:
        """
        Get MNDO version number of given path
        """
        string = os.path.join(path, "mndo") + " nonexisting"
        with sp.Popen(string, shell=True, stdout=sp.PIPE, stderr=sp.PIPE) as proc:
            comm = proc.communicate()[0].decode()
            if not comm:
                raise ValueError("MNDO version not found!")
