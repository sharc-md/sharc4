...
class TINKER(INTERFACE):
    ...
    def read_template(self, template_filename='TINKER.template'):
        ...
        QMin['mmatoms'] = [
            MMATOM(i, v[0].lower() == 'qm', QMin['elements'][i], [0., 0., 0.], v[1], set(v[2:]))
            for i, v in enumerate(QMin['template']['qmmm_table'])
        ]
        print(len(QMin['mmatoms']))
        # sanitize mmatoms
        # set links
        qm = []
        mm = []
    ...
    def read_resources(self, resources_filename='TINKER.resources'):
        ...
        empty_paths = list(filter(lambda x: not (bool(QMin['resources'][x]) or QMin['resources'][x].isspace()), paths.keys()))
