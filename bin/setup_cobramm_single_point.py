...
def displaywelcome():
    ...
    string += '  ' + '=' * 80 + '\n'
    string += '||' + '{:^80}'.format('') + '||\n'
    string += '||' + '{:^80}'.format('Setup QM/MM single points with SHARC/COBRAMM') + '||\n'
    string += '||' + '{:^80}'.format('') + '||\n'
    ...
    string += '  ' + '=' * 80 + '\n\n'
    string += '''
This script automatizes the setup of the input files for SHARC/COBRAMM QM/MM single point calculations.
  '''
    print(string)
...
def close_keystrokes():
    KEYSTROKES.close()
    shutil.move('KEYSTROKES.tmp', 'KEYSTROKES.setup_cobramm_single_point')
...
def get_general():
    ...
    INFOS['ngeom'] = ngeoms
    INFOS['natom'] = natom
    print('\nGeometry is read in %s\n' % (path))

    print('{:-^60}'.format('Full system topology file'))
    ...
    print('{:-^60}'.format('QM system topology file'))
    ...
    print('{:-^60}'.format('Layer definition file'))
    ...
    string = '\n  ' + '=' * 80 + '\n'
    string += '||' + '{:^80}'.format('COBRAMM Interface setup') + '||\n'
    string += '  ' + '=' * 80 + '\n\n'
    print(string)

    print('{:-^60}'.format('Path to COBRAMM') + '\n')
    ...
    if path == '':
        ...
        path = '$AMBERHOME/'
    print('\nPlease specify path to AMBER directory (SHELL variables and ~ can be used, will be expanded when interface is started).\n')
    INFOS['amber'] = question('Path to AMBER:', str, path)


    print('{:-^60}'.format('Scratch directory') + '\n')
    print('Please specify an appropriate scratch directory\n. This will contain inside a subdirectory QM.\nThis will be used to temporally store the integrals. The scratch directory will be deleted after the calculation.\nRemember that this script cannot check whether the path is valid, since you may run the calculations on a different machine. The path will not be expanded by this script.')
    INFOS['scratchdir'] = question('Path to scratch directory:', str)
    print('')

    print('{:-^60}'.format('COBRAMM input template file') + '\n')
...
''')
    ...
    if 'cobramm.template' not in INFOS:
        while True:
    ...
    print('{:-^60}'.format('COBRAMM Ressource usage') + '\n')
...
def main():
    ...
    usage = '''
python setup_cobramm_single_point.py

This interactive program prepares SHARC/COBRAMM single point calculations.
'''
