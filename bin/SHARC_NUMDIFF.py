...
class SHARC_NUMDIFF(SHARC_HYBRID):
    ...
    _changelogstring = CHANELOGSTRING
    ...
    def get_features(self, KEYSTROKES: TextIOWrapper | None = None):
        ...
        if not self._read_template:
            self.template_file = question(
                "Please specify the path to your NUMDIFF.template file", str, KEYSTROKES=KEYSTROKES, default="NUMDIFF.template"
            )
        ...
        for k, v in needed.items():
            if all(j in ref_features for j in v):
                possible.add(k)
    ...
    def read_displacement_coordinates(self, disp_coord_filename: str) -> None:
        ...
        # Read the coord file
        disp_coords = []
        with open(disp_coord_filename, "r", encoding="utf-8") as f:
            for line in f:
                if "units" in line:
                    line = f.readline()
    ...
    def setup_interface(self) -> None:
        ...
        # paths
        self.QMin.resources["scratchdir"] = os.path.abspath(
            os.path.expanduser(os.path.expandvars(self.QMin.resources["scratchdir"]))
        )
        if self.QMin.template["qm-dir"] is None:
            raise ValueError("Keyword 'qm-dir' not found in template file!")
        qmdir = os.path.abspath(os.path.expanduser(os.path.expandvars(self.QMin.template["qm-dir"])))
        qm_program = self.QMin.template["qm-program"]
        ...
        self.ref_interface = self._load_interface(qm_program)(
            logfile=ref_logfile, logname=ref_logname, loglevel=self.log.level, persistent=False
        )
        # if isinstance(self.ref_interface, SHARC_HYBRID):
        #    self.log.error('Currently, Hybrid interfaces cannot be used as children of SHARC_NUMDIFF.py')
        #    raise NotImplementedError
        ...
        for i in check_these:
            if i in self.QMin.template["whitelist"]:
                if i in ref_features:
                    self.log.info(f"Request {i} white-listed and available from child, will be passed to reference")
                else:
                    self.do_numerically.add(i)
                    self.log.info(f"Request {i} white-listed but not available from child, will be done numerically")
    ...
    def run(self) -> None:
        ...
        # coordinates
        self.ref_interface.set_coords(self.QMin.coords["coords"].copy(), False)
        ...
        self.num_requests["step"] = self.QMin.save["step"]
        ref_requests["step"] = self.QMin.save["step"]

        # mandatory requests
        ref_requests["nooverlap"] = False
        ref_requests["cleanup"] = False
        ...
        if do_numerically:
            ...
            for child in self._kindergarden.values():
                ...
                cleandir(child.QMin.save["savedir"])
                ...
                for f in ls:
                    ...
                    tofile = os.path.join(child.QMin.save["savedir"], f)
                ...
                if self.QMin.template["qm-program"].upper() == "LEGACY":
                    for f in ["MOLPRO", "COLUMBUS", "ADF_AMS", "BAGEL"]:
                        ...
                        if os.path.isdir(fromdir):
                            ...
                            todir = os.path.join(child.QMin.save["savedir"], f)
                            shutil.copytree(fromdir, todir)
                # set step for displaced child
                child_requests["step"] = self.ref_interface.QMin.save["step"] + 1
                stepfile = os.path.join(child.QMin.save["savedir"], "STEP")
                writefile(stepfile, str(self.ref_interface.QMin.save["step"]))
                child.read_requests(child_requests)
            ...
            self.log.info(f"FINISH:\t{t2}\tRuntime: {t2-t1}\n")
                            ...
                            for request in self.num_requests:
                                ...
                                match self.QMin.template["numdiff_method"]:
                                    ...
                                    case "central-quad":
                                        ...
                                        denomimator = 6.0 * stepsize * (displacements["p"] - displacements["n"])
                                        result = numerator / denomimator
                                    case _:
                                    ...
                                    case "nacdr":  # QMout['socdr'] has shape (nstates, nstates, natom, 3)
                                        match self.QMin.template["numdiff_representation"]:
                                            case "adiabatic":
                                                ...
                                                self.QMout["nacdr"][:, :, iatom, cart_directions[idir]] = (
                                                    result - result.T
                                                ) / 2.0
                                            case "diabatic":
                                                # result contains dH/dR. To get the NAC, we need to scale by the energy gaps
                                                result = (result + result.T) / 2.0  # maybe unnecessary
