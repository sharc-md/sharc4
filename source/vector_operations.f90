...
contains
    ...
    function norm(vec1) result(norm_vec1)
        real(8), dimension(:) :: vec1
        real(8), allocatable, dimension(:) :: norm_vec1
        allocate(norm_vec1(3))

        norm_vec1 = vec1/sqrt(sum(vec1*vec1))
    end function norm

    function z3gram_schmidt_vec1_on_vec2(vec1, vec2) result(new_vec1)
        real(8), dimension(:) :: vec1, vec2
        real(8), allocatable, dimension(:) :: f, new_vec1
        
        allocate(f(3))
        allocate(new_vec1(3))
        
        f = sum(vec2*vec1) / sum(vec2*vec2)
        new_vec1 = vec1 - f * vec2
        new_vec1 = new_vec1 / sqrt(sum(new_vec1*new_vec1))
