...
module data_extractor_NetCDFmodule
  ...
  type Tshdata
    ...
    complex*16, allocatable :: H_MCH_ss(:,:),U_ss(:,:),DM_ssd(:,:,:),MDM_ssd(:,:,:),EQM_ssdd(:,:,:,:)
  ...
  !> # Information which is calculated at each time step
    ...
    complex*16, allocatable :: laser_efield_tp(:,:)        !< laser field for all timesteps
    complex*16, allocatable :: laser_bfield_tp(:,:)        !< laser field for all timesteps
    complex*16, allocatable :: laser_egrad_tpd(:,:,:)        !< laser field for all timesteps
    ...
    real*8,allocatable :: expec_mdm(:)               !< oscillator strength per state
    real*8,allocatable :: expec_mdm_mch(:)           !< oscillator strength per state in MCH basis
    real*8,allocatable :: expec_eqm(:)               !< oscillator strength per state
    real*8,allocatable :: expec_eqm_mch(:)           !< oscillator strength per state in MCH basis
...
! -----------------------------------------------------------------------------
  ...
  write_options%write_mag_dipact    = .false.
  write_options%write_el_quadact    = .false.
...
! -----------------------------------------------------------------------------
  ...
  if (is_integer) then
    ...
    allocate( shdata%MDM_ssd(nstates,nstates,3) )
    allocate( shdata%EQM_ssdd(nstates,nstates,3,3) )
    ...
    allocate( shdata%expec_mdm(nstates),shdata%expec_mdm_mch(nstates),shdata%expec_mdm_act(nstates) )
    allocate( shdata%expec_eqm(nstates),shdata%expec_eqm_mch(nstates),shdata%expec_eqm_act(nstates) )
...
! -----------------------------------------------------------------------------
  ...
  if (write_options%write_el_quadact) then
...
! -----------------------------------------------------------------------------
  ...
  subroutine open_output_files(write_options)
      ...
      if (write_options%write_mag_dip)   open(unit=u_mdm, file='output_data/fosc_md2.out', status='replace', action='write')          ! -dmd
      if (write_options%write_el_quad)   open(unit=u_eqm, file='output_data/fosc_eq2.out', status='replace', action='write')          ! -deq
      ...
      if (write_options%write_mag_dipact)    open(unit=u_fosc_act_mdm, file='output_data/fosc_act_md2.out', status='replace', action='write')     ! -damd
      if (write_options%write_el_quadact)    open(unit=u_fosc_act_eqm, file='output_data/fosc_act_eq2.out', status='replace', action='write')     ! -daeq
...
! -----------------------------------------------------------------------------
  ...
  subroutine read_properties_from_output(nstates, step, u_dat, general_infos, prop_info, shdata, ierr)
    ...
    enddo
    do idir=1,3
      call matread(nstates,shdata%MDM_ssd(:,:,idir),u_dat,string1)
    enddo
    do idir=1,3
      do jdir=1,3
        call matread(nstates,shdata%EQM_ssdd(:,:,idir,jdir),u_dat,string1)
      enddo
    enddo
...
! -----------------------------------------------------------------------------
  ...
  subroutine write_data_to_file(nstates, step, general_infos, write_options, shdata)
    ...
    if (write_options%write_geometry) then
      ...
      write(u_xyz,'(A5, 1X, F14.5, 1X, I4, 1X, i4)') 't= ',shdata%time_step*general_infos%dtstep, &
      &shdata%state_diag, shdata%state_MCH
    ...
    if (write_options%write_energy) then
      ...
      &shdata%time_step*general_infos%dtstep+shdata%time_shift, shdata%Ekin*au2eV, shdata%Epot*au2eV, (shdata%Epot+shdata%Ekin)*au2eV,&
    ...
    if (write_options%write_dip) then
      ...
      &shdata%time_step*general_infos%dtstep+shdata%time_shift, shdata%expec_dm(shdata%state_diag),&
      (shdata%expec_dm(istate),istate=1,nstates)
    endif
    if (write_options%write_mag_dip) then
      ! write to fosc_md2.out
      write(u_mdm,'(2X,1000(ES20.12E3,1X))') &
      &shdata%time_step*general_infos%dtstep+shdata%time_shift, shdata%expec_mdm(shdata%state_diag),&
      (shdata%expec_mdm(istate),istate=1,nstates)
    endif
    if (write_options%write_el_quad) then
      ! write to fosc_eq2.out
      write(u_eqm,'(2X,1000(ES20.12E3,1X))') &
      &shdata%time_step*general_infos%dtstep+shdata%time_shift, shdata%expec_eqm(shdata%state_diag),&
      (shdata%expec_eqm(istate),istate=1,nstates)
    endif
    if (write_options%write_dipact)  then
      ! write to fosc_act.out
      write(u_fosc_act,'(2X,1000(ES20.12E3,1X))') &
      & shdata%time_step*general_infos%dtstep+shdata%time_shift,(abs(real(shdata%H_diag_ss(istate,istate)&
...
!       (expec_dm_act(istate),istate=1,nstates)
    ...
    if (write_options%write_dipoles) then
      ...
      &shdata%time_step*general_infos%dtstep+shdata%time_shift, shdata%dipole_diag(shdata%state_diag),&
    ...
    if (write_options%write_mag_dipact)  then
      ...
      & (shdata%expec_dm_act(istate),istate=1,nstates)
!       write(u_fosc_act,'(2X,1000(ES20.12E3,1X))') &
!       &shdata%time_step*general_infos%dtstep+shdata%time_shift,(real(H_diag_ss(istate,istate)-H_diag_ss(state_diag,state_diag))*au2eV,istate=1,nstates),&
!       (expec_dm_act(istate),istate=1,nstates)
    endif
    if (write_options%write_el_quadact)  then
      ! write to fosc_act.out
      write(u_fosc_act_eqm,'(2X,1000(ES20.12E3,1X))') &
      & shdata%time_step*general_infos%dtstep+shdata%time_shift,(abs(real(shdata%H_diag_ss(istate,istate)&
      & -shdata%H_diag_ss(shdata%state_diag,shdata%state_diag)))*au2eV,istate=1,nstates),&
      & (shdata%expec_eqm_act(istate),istate=1,nstates)
!       write(u_fosc_act,'(2X,1000(ES20.12E3,1X))') &
!       &shdata%time_step*general_infos%dtstep+shdata%time_shift,(real(H_diag_ss(istate,istate)-H_diag_ss(state_diag,state_diag))*au2eV,istate=1,nstates),&
!       (expec_dm_act(istate),istate=1,nstates)
    endif
    ...
    if (write_options%write_iondiag) then
      ...
      & shdata%time_step*general_infos%dtstep+shdata%time_shift,shdata%state_diag,(real(shdata%H_diag_ss(istate,istate)&
      & -shdata%H_diag_ss(shdata%state_diag,shdata%state_diag))*au2eV,istate=1,nstates),&
    ...
    if (write_options%write_ionmch) then
      ...
      & shdata%time_step*general_infos%dtstep+shdata%time_shift,shdata%state_diag,(real(shdata%H_mch_ss(istate,istate) &
    ...
    if (write_options%write_spin) then
      ...
      &shdata%time_step*general_infos%dtstep+shdata%time_shift, shdata%expec_s(shdata%state_diag),&
    ...
    if (write_options%write_coeffdiag) then
      ...
      &shdata%time_step*general_infos%dtstep+shdata%time_shift, sumc,&
      (shdata%coeff_diag_s(istate),istate=1,nstates)
      ! write to coeff_class_diag.out
      write(u_classd,'(2X,1000(ES20.12E3,1X))') &
      &shdata%time_step*general_infos%dtstep+shdata%time_shift, 1.d0,&
    ...
    if (write_options%write_prob) then
      ...
      &shdata%time_step*general_infos%dtstep+shdata%time_shift, shdata%randnum,&
...
! -----------------------------------------------------------------------------
    ...
    subroutine process_data(nstates, step, general_infos, write_options, shdata, prop_info)
        ...
        use definitions, only: alpha, au2debye
        implicit none
  integer, intent(in) :: nstates, step
  type(Tgeneral_infos), intent(in) :: general_infos
  type(Twrite_options), intent(in) :: write_options
...
!     compute density matrix
    ...
    if (write_options%write_mag_dip .or. write_options%write_mag_dipact .or. write_options%write_expec .or.&
        &write_options%write_expecmch) then
      shdata%expec_mdm=0.d0
      shdata%expec_mdm_mch=0.d0
      shdata%expec_mdm_act=0.d0 
      do idir=1,3
        shdata%A_ss=shdata%MDM_ssd(:,:,idir)
        shdata%expec_mdm_mch=shdata%expec_mdm_mch+real(shdata%A_ss(:,1)*shdata%A_ss(1,:))
        call transform(nstates,shdata%A_ss,shdata%U_ss,'utau')
        shdata%expec_mdm=shdata%expec_mdm+real(shdata%A_ss(:,1)*shdata%A_ss(1,:))
        shdata%expec_mdm_act=shdata%expec_mdm_act+real(shdata%A_ss(:,shdata%state_diag)*shdata%A_ss(shdata%state_diag,:))
      enddo 
      shdata%expec_mdm=shdata%expec_mdm*2./3.
      shdata%expec_mdm_mch=shdata%expec_mdm_mch*2./3.
      shdata%expec_mdm_act=shdata%expec_mdm_act*2./3.  
      do i=1,nstates
        shdata%expec_mdm(i)=shdata%expec_mdm(i)*real(shdata%H_diag_ss(i,i)-shdata%H_diag_ss(1,1))
        shdata%expec_mdm_mch(i)=shdata%expec_mdm_mch(i)*real(shdata%H_MCH_ss(i,i)-shdata%H_MCH_ss(1,1))
        shdata%expec_mdm_act(i)=shdata%expec_mdm_act(i)*real(shdata%H_diag_ss(i,i)-shdata%H_diag_ss(shdata%state_diag,shdata%state_diag))
      enddo 
    endif
    
    ! calculate oscillator strengths for electric quadrupoles
    if (write_options%write_el_quad .or. write_options%write_el_quadact .or. write_options%write_expec .or.&
        &write_options%write_expecmch) then
      shdata%expec_eqm=0.d0
      shdata%expec_eqm_mch=0.d0
      shdata%expec_eqm_act=0.d0 
      do idir=1,3
        do jdir=1,3
          shdata%A_ss=shdata%EQM_ssdd(:,:,idir,jdir)
          shdata%expec_eqm_mch=shdata%expec_eqm_mch+real(shdata%A_ss(:,1)*shdata%A_ss(1,:))
          call transform(nstates,shdata%A_ss,shdata%U_ss,'utau')
          shdata%expec_eqm=shdata%expec_eqm+real(shdata%A_ss(:,1)*shdata%A_ss(1,:))
          shdata%expec_eqm_act=shdata%expec_eqm_act+real(shdata%A_ss(:,shdata%state_diag)*shdata%A_ss(shdata%state_diag,:))
        enddo
      enddo 
      shdata%expec_eqm=shdata%expec_eqm*1./20.*alpha**2
      shdata%expec_eqm_mch=shdata%expec_eqm_mch*1./20.*alpha**2
      shdata%expec_eqm_act=shdata%expec_eqm_act*1./20.*alpha**2 
      do i=1,nstates
        shdata%expec_eqm(i)=shdata%expec_eqm(i)*real(shdata%H_diag_ss(i,i)-shdata%H_diag_ss(1,1))
        shdata%expec_eqm_mch(i)=shdata%expec_eqm_mch(i)*real(shdata%H_MCH_ss(i,i)-shdata%H_MCH_ss(1,1))
        shdata%expec_eqm_act(i)=shdata%expec_eqm_act(i)*real(shdata%H_diag_ss(i,i)-shdata%H_diag_ss(shdata%state_diag,shdata%state_diag))
      enddo
    endif
