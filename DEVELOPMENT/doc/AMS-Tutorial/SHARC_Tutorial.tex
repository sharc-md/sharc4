...
The sulfur dioxide molecule presents a simple yet interesting model system. It undergoes intersystem crossing and will populate a triplet state with increasing simulation time.
...
\end{Verbatim}
You can find a \ttt{AMS.template} file in the examples (\ttt{\$SHARC/../examples/SHARC\_AMS/AMS.template}), where we wrote every possible option that you could put in there with explanatory comments (see below).
\begin{oframed}
\footnotesize\begin{Verbatim}[commandchars=\\\{\}]
...
However, you only need to type the following into your \ttt{AMS.template file}
\begin{oframed}
  \begin{Verbatim}[commandchars=\\\{\}]
relativistic scalar zora 
basis DZP
...
\end{Verbatim}
Make sure that you have all required files (\ttt{initconds.excited}, \ttt{AMS.template}) in this directory.
...
There are two sharp switches between the population of the $T_2$ and the $T_3$ and back in the end, which is an unavoided crossing of the uncoupled ${}^3B_2$ and ${}^3B_1$ states. 
...
Fourth, we will obtain the populations of the diabatic states during the simulations. for this we start the \ttt{populations.py} script again.
\begin{Verbatim}[commandchars=\\\{\}]
  user@host> \textcolor{red}{$SHARC/populations.py}
\end{Verbatim}
...
In figure~\ref{fig:pop_class} it can be seen that in the first 15~fs population is transferred form $S_1$ to $S_2$, before the decoherence correction forces the ensemble into the $S_1$ state again. Afterwards, one trajectory transitions from the initial $S_1$ state to the $T_2$ state with a brief section in the $T_3$ due to an unavoided crossing.
...
The chosen interval limits were 0.000001 and 0.0001, giving three classes of states (below 0.0000001, between 0.000001 and 0.0001, and above 0.0001). 
Initially, all trajectories are classified into the third class. 
During the dynamics, the dissociations/torsions/hops reduce the oscillator strength, so that the trajectories are classified into the second class.
Later, some trajectories transition on to the triplet state which has an even lower oscillator strength (first class). However, also the singlet states have oscillator strengths below 0.000001 a some parts of the simulation, so that there is no clear separation between them and the triplet states. 
...
In figure~\ref{fig:pop_diab}, it can be seen that the population shifts from the ${}^1B_1$ state to the ${}^1A_2$ state in the beginning. After 45~fs, the ${}^3B_2$ state (all three magnetic quantum numbers) are increasingly populated.
...
\begin{figure}[ptb]
  \centering
  \includegraphics[width=0.8\textwidth]{figures/pop_fosc.png}
  \caption{Populations classified based on oscillator strength for an ensemble of 6 trajectories.}
  \label{fig:pop_fosc}
\end{figure}
\begin{figure}[ptb]
  \centering
...
\begin{figure}[ptb]
  ...
  \caption{Total activity of the normal modes of \ce{SO2}, where mode 1 shows the highest activity.}
  \label{fig:nma}
\end{figure}
...
Here, we will simulate the transient emission spectrum for \ce{SO2}, although the spectrum could be incomplete due to the missing of higher excited states.
...
\begin{figure}[h]
  ...
  \caption{Simulated transient absorption spectrum of \ce{SO2}. Ground state bleech (negative absorption) is shown in red, and excited-state absorption (positive absorption) is shown in blue.}
  \label{fig:TAS}
\end{figure}
...
To continue, we need the \ttt{AMS.freq.molden} and the \ttt{AMS.template} files obtained in Chapter~\ref{chap:full}.
...
There can be 121 trajectories set up.

Please enter the index of the first initial condition in the initconds file to be setup.
Starting index: [1] \textbf{\textcolor{red}{<ENTER>}}

There can be 121 trajectories set up, starting in 1 states.

Please enter the total number of trajectories to setup.
Number of trajectories: [121] \textbf{\textcolor{red}{<ENTER>}}
...
isactive                   [True, True, True, True, True, True, True, True, True, True, True, True]
show_content               False
n_issel                    [0, 121, 39, 0, 0, 0, 0, 0, 0, 0, 0, 0]
setupstates                {2}
firstindex                 1
ntraj                      121
interface                  7
...
stride                     [1]
printlevel                 2
cwd                        /user/severin/workdir/ams_sharc_tutorial2/Tutorial/LVC/NetCDF/traj
here                       True
copydir                    /user/severin/workdir/ams_sharc_tutorial2/Tutorial/LVC/NetCDF/traj
qsub                       False
...
121 trajectories setup, last initial condition was 198 in state 2.
\end{Verbatim}
\end{oframed}
...
Note also that we will simulate 121 trajectories for 700~fs to demonstrate the high computational efficiency of the method.
