...
\textbf{The first one} is a quick tutorial, just presenting the files necessary to run a single \sharc\ trajectory, without initial condition generation, ensemble management, or analysis.

\textbf{The second part} contains the full tutorial presenting a complete dynamics study including initial condition generation, ensemble management, and trajectory analysis (with plotting of the results and a brief discussion). 
This tutorial will use the interface to \textsc{Molcas}.
\textbf{Note that \textsc{OpenMolcas} is freely available, and hence users who want to follow the tutorial as closely as possible are advised to get an \textsc{OpenMolcas} version compatible with \sharc\ from the \link{https://sharc-md.org/?page_id=335}{\todo{\sharc\ homepage}}.}

\textbf{The third part} is a collection of more specialized tutorials showing some advanced usage aspects in detail.

\textbf{The fourth part} contains mini-tutorials for using the other quantum chemistry interfaces besides the one for \textsc{Molcas}.
...
The only exception to this is that the results might be different if you employed a different compiler to compile \ttt{sharc.x} (this tutorial uses \ttt{gfortran 4.8.5}) or if you use a different \textsc{Molcas} version (\todo{this tutorial uses XXXXX}).
...
Prepare a geometry file called \ttt{geom.xyz} containing the geometry given above.
...
Make sure that you have all required files (\ttt{initconds.excited}, \ttt{MOLCAS.template}, \ttt{MOLCAS.RasOrb}) at hand.
\begin{Verbatim}[commandchars=\\\{\}]
...
user@host> \textbf{\textcolor{red}{python $SHARC/geo.py -t 0.5 -g $i/output.xyz < Geo.inp > $i/Geo.out;}}
...
\refermanual{m-sec:QMout2LVC.py}
