\documentclass[a4paper,10pt,DIV=15,openany,twoside=false]{scrbook}
...
In order to use the analysis scripts \ttt{trajana\_essdyn.py} and \ttt{trajana\_nma.py}, it is necessary to set the Python path accordingly:
\begin{example}
\verb|export PYTHONPATH=$PYTHONPATH:$SHARC/../lib/|
\end{example}
...
Based on a set of costumizable criteria, the script determines for each trajectory a ``maximum usable time''.
...
Trajectories can also be manually excluded from analysis, by creating a file called \ttt{CRASHED} or \ttt{RUNNING} in the respective directory.

After the trajectories are prepared for statistical analysis, the script \ttt{populations.py} (section~\ref{sec:populations.py}) can calculate average excited-state populations. 
...
\begin{tabular}{>{\ttfamily}lp{9.5cm}r}
...
% ========================================
  ...
  \hline
  no\_hop               &                                       &Disables surface hopping.\\
\end{longtable}
}
...
\begin{table}
  ...
  samestep        &Specifies that this is an additional calculation at the same geometry/timestep. Implies that, e.g., the converged orbitals from the savedir could be reused or that the wavefunction calculations could be skipped.\\
...
\begin{table}
  ...
  \centering
  \caption{Overview over capabilities of \sharc\ interfaces.}
  \label{tab:interfaces}
  \begin{tabular}{llccccccc}
    \toprule
    Method          &Program          &Mults.  &SOC     &DM      &Grad    &NACDR   &OVL     \\
    \midrule
    SA-CASSCF   &\textsc{Molpro}      &any     &\chk    &\chk    &\chk    &\chk    &\chk    \\
                &\textsc{Molcas}      &any     &\chk    &\chk    &\chk    &        &\chk    \\
                &\textsc{Columbus}    &any     &(\chk)  &\chk    &\chk    &(\chk)  &\chk    \\
    MS-CASPT2   &\textsc{Molcas}      &any     &\chk    &\chk    &(\chk)  &        &\chk    \\
    MR-CISD     &\textsc{Columbus}    &any     &(\chk)  &\chk    &\chk    &(\chk)  &\chk    \\
    TD-DFT      &\textsc{ADF}         &S, T    &\chk    &(\chk)  &\chk    &        &\chk    \\
    ADC(2)      &\textsc{Turbomole}   &S, T    &\chk    &\chk    &\chk    &        &\chk    \\
    CC2         &\textsc{Turbomole}   &S, T    &        &\chk    &\chk    &        &\chk    \\
    Analytical  &---                  &any     &\chk    &\chk    &\chk    &        &\chk    \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}
  ...
  \begin{tabular}{llll}
    ...
    \midrule
    \textsc{Molpro}     &\ttt{MOLPRO.template}  &\ttt{SH2PRO.inp}       &\ttt{wf.init}                  \\
                        &                       &                       &\ttt{wf.<job>.init}            \\
    \textsc{Molcas}     &\ttt{MOLCAS.template}  &\ttt{SH2CAS.inp}       &\ttt{MOLCAS.<mult>.RasOrb.init}\\
                        &                       &                       &\ttt{MOLCAS.<mult>.JobIph.init}\\
    \textsc{Columbus}   &a directory            &\ttt{SH2COL.inp}       &\ttt{mocoef\_mc.init}          \\
                        &                       &                       &\ttt{mocoef\_mc.init.<job>}    \\
                        &                       &                       &\ttt{molcas.RasOrb.init}       \\
                        &                       &                       &\ttt{molcas.RasOrb.init.<job>} \\
    \textsc{ADF}        &\ttt{ADF.template}     &\ttt{SH2ADF.inp}       &\ttt{ADF.t21\_init}            \\
    \textsc{Turbomole}  &\ttt{RICC2.template}   &\ttt{SH2CC2.inp}       &\ttt{mos.init}                 \\
    Analytical          &\ttt{SH2Ana.inp}       &                       &N/A                            \\
    \bottomrule
  \end{tabular}
\end{table}
...
The \sharc-\textsc{Molpro} interface allows to run \sharc\ dynamics with \textsc{Molpro}'s CASSCF wavefunctions. RASSCF is not supported, since on RASSCF level state-averaging over different multiplicities is not possible. The interface uses \textsc{Molpro}'s CI program in order to calculate transition dipole moments and spin-orbit couplings. Gradients and non-adiabatic coupling vectors are calculated using \textsc{Molpro}'s CADPACK code (hence no generally contracted basis sets can be used). In the new version of the interface, overlaps are calculated using the \textsc{Wfoverlap} code. 
Time-derivative couplings (NACDT) are not supported anymore in the \sharc-\textsc{Molpro} interface.
Wavefunction phases between the CASSCF and MRCI wavefunctions are automatically adjusted.
The interface can trivially parallelize the computation of gradients and coupling vectors over several processors.
% Parallel execution is possible if \textsc{Molpro} is properly installed in parallel mode.

The \sharc-\textsc{Molpro} interface needs two additional input files, which should be present in \ttt{QM/}. Those input files are \ttt{SH2PRO.inp}, which contains, e.g., the paths to \textsc{Molpro} and the scratch directory, and \ttt{MOLPRO.template}, which is a keyword-argument input file specifying the CASSCF level of theory. 
If \ttt{QM/wf.init} is present, it will be used as a \textsc{Molpro} wavefunction file containing the initial MOs.
...
In the new version, the template file is a keyword-argument list file, similar to the template file for the \textsc{Molcas} interface (see section~\ref{sec:int:molcas}).
...
basis_external /path/to/basisset        # no spaces allowed
dkho 2
...
Note how for different jobs it is possible to have different active spaces and state-averagings.
...
Remember that \ttt{molcas\_input.py} cannot create multi-job templates.
...
\begin{table}[t]
  \centering
  \caption{Keywords for the \ttt{SH2PRO.inp} input file.}
  ...
  ncore           &Number of neglected core orbitals for overlap and Dyson calculations.\\
  ndocc           &Number of doubly occupied orbitals for Dyson calculations.\\
...
Mandatory keywords are the paths to \textsc{Molpro}, the scratch directory, and to the \textsc{Wfoverlap} executable (the latter for overlap, Dyson, NACDR calculations).
...
Note that in the new version of the interface, \textsc{Molpro} calls iteself cannot be run with multiple CPUs.
...
For CASSCF calculations it is always a good idea to start from converged MOs from a nearby geometry. For the first timestep, if a file \ttt{QM/wf.init} is present, the \sharc-\textsc{Molpro} interface will take this file for the starting orbitals. In case of multi-job calculations, separate initial orbitals can be provided with files called \ttt{QM/wf.<job>.init}, where \ttt{<job>} is an integer.
...
Since \textsc{Molcas} is not able to calculate the full non-adiabatic coupling vectors, only wavefunction overlaps are currently implemented in the interface (overlaps are calculated via RASSI). 
...
always\_orb\_init &Do not use the orbital guesses from previous calculations/timesteps, but always use the provided initial orbitals.\\
always\_guess   &Always use the orbital guess from \textsc{Molcas}'s \ttt{guessorb} module.\\
displ           &Cartesian displacement (in \AA) for numerical differentiation.\\
...
auxbasis        &The auxilliary basis set used in \ttt{ricc2}. If no auxbasis is given, the interface will let \ttt{define} decide on a suitable auxbasis.\\
...
The purpose of this script is to automatize the crucial step of checking the trajectories in an ensemble before beginning the ensemble analysis.

The tool can check several different aspects of the trajectories.
First, it checks whether all relevant output files of the trajectories are present.
...
It also checks for conservation of total population, and for trajectories ran with overlap coupling also intruder states are checked.

Note that the diagnostics script also automatically runs the \ttt{data\_extractor.x} for all trajectories.
...
\begin{table}
  ...
  \begin{tabular}{>{\ttfamily}lcp{10cm}}
    \toprule
    Key  &Value     &Explanation\\
    \midrule
    missing\_output     &Boolean        &Checks if "output.lis", "output.log", "output.xyz", "output.dat" are existing.\\
    missing\_restart    &Boolean        &Checks if "restart.ctrl", "restart.traj", "restart/" are existing.\\
    normal\_termination &Boolean        &Checks for exit status of trajectory (RUNNING, CRASHED, FINISHED).\\
    etot\_window        &Float          &Maximum permittible drift in total energy (in eV).\\
    etot\_step          &Float          &Maximum permittible total energy difference between two successive timesteps (in eV).\\
    epot\_step          &Float          &Maximum permittible active state potential energy difference between two successive timesteps (in eV). Not checked for timesteps where a hop occurred.\\
    ekin\_step          &Float          &Maximum permittible kinetic energy difference between two successive timesteps (in eV).\\
    pop\_window         &Float          &Maximum permittible drift in total population.\\
    hop\_energy         &Float          &Maximum permittible change in active state energy difference during a surface hop (in eV).\\
...
This value is either the total simulation time or the time when the first violation (of total energy conservation, potential/kinetic energy smoothness, hopping energy restriction, or intruder state) in the trajectory appeared.
The script prints the $T_\text{mu}$ values for all trajectories at the end.

The user can then give a threshold for $T_\text{mu}$, so that \ttt{diagnostics.py} excludes all trajectories with values smaller than the threshold from analysis (the script will create a file \ttt{DONT\_ANALYZE} into each affected trajectory).
...
A small value of the threshold will mean that many trajectories are admitted for analysis, giving good statistics, but only for very short simulations.
On the other hand, choosing a large threshold will lead to a satisfactory simulation time, but only few trajectories will be included in the analysis (only the ones long enough).

It is advisable that the chosen threshold value is used as input for the other analysis scripts which ask for a maximum analysis time (\ttt{populations.py}, \ttt{transition.py}, \ttt{crossing.py}, \ttt{trajana\_essdyn.py}, \ttt{trajana\_nma.py}).
...
\begin{table}
  ...
  \begin{tabular}{lp{7cm}>{\ttfamily}lc}
    \toprule
...
Given a matrix $\VEC{Q}$ containing the normal mode vectors and a reference geometry $\VEC{R}_\text{ref}$, calculate for each trajectory
\begin{equation}
