...
\begin{itemize}
  ...
  \item Calculation of Dyson norms for single-photon ionization spectra (for some interfaces) \cite{Ruckenbauer2016SR}.
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item Much better wave function and $\VEC{U}$ matrix phase tracking.
    \item Support for on-the-fly computation of Dyson norms and \textsc{TheoDORE} descriptors.
    \item Option to gracefully stop trajectories.
  \end{itemize}
  \item Fully integrated, efficient wave function overlap program \ttt{wfoverlap.x}
  \item Quantum chemistry interfaces:
  \begin{itemize}
    \item \textsc{Molpro}: overhauled, uses \ttt{wfoverlap.x}, gives consistent phase between CASSCF and CI wave functions, can do Dyson norms.
    \item \textsc{Molcas}: overhauled, can do (MS)-CASPT2 (numerical gradients), QM/MM, Cholesky, Dyson norms, is parallelized, \todo{works with \textsc{Molcas} version XXX}.
    \item \textsc{Columbus}: overhauled, works with \ttt{wfoverlap.x}, can use \textsc{Dalton} integrals, can use \textsc{Molcas} orbitals, can do Dyson norms.
    \item \textsc{Analytical}: ---
    \item \textsc{Turbomole}: new interface ,can do ADC(2) and CC2; has SOC, overlaps, works with \textsc{TheoDORE}.
    \item \textsc{ADF}: new interface, can do TD-DFT; has SOC, overlaps, Dyson norms, QM/MM, works with \textsc{TheoDORE}.
    \item \textsc{Gaussian}: new interface, can do TD-DFT; has overlaps, Dyson norms, works with \textsc{TheoDORE}.
    \item LVC: new interface, can do (analytical) linear vibronic coupling models.
  \end{itemize}
  \item Auxilliary scripts:
  \begin{itemize}
    \item \ttt{wigner.py}: elevated temperature sampling.
    ...
    \item \ttt{spectrum.py}: log-normal convolution, density of states.
    ...
    \item \ttt{make\_fitscript.py}: new script, prepares kinetic model fits to obtain time constants.
    ...
    \item \ttt{orca\_External}: new script, allows crossing point optimization with any method interfaced in \sharc.
    \item Several input generation helpers.
  \end{itemize}
  \item Reworked tutorial using \textsc{Molcas} (which is free).
\end{itemize}
...
Applications of the \sharc\ code can be found in Refs.~\cite{Richter2012JPCL, Mai2013C, Mai2014TCC, Mai2014JCP_SO2, Gonzalez2014, Richter2014PCCP,Martinez-Fernandez2014JCTC, Crespo-Hernandez2015JACS, Marazzi2016JPCL, Richter2016JCTC, Cao2016CP, Banerjee2016PCCP, Mai2016JPCL, Mai2016NC, Peccati2016PTRSA, Borin2017PCCP, Mai2017CP, Siouri2017JPCA, Bellshaw2017CPL, Sun2017JPCA, Rauer2016JACS, Atkins2017JPCL, Schnappinger2017PCCP, Mai2017JCP}.
...
\todo{Others:}
\begin{itemize}
  \item \todo{\textsc{TheoDORE}: \cite{Plasser2014JCP1, Plasser2014JCP2, Plasser2017TheoDORE}}
  \item \todo{\textsc{Wfoverlap}: \cite{Plasser2016JCTC, Plasser2016JCP}}
\end{itemize}


\section{Authors}

The current version of the \sharc\ suite has been programmed by Sebastian Mai, Martin Richter, Andrew Atkins, Felix Plasser, Moritz Heindl, and Philipp Marquetand of the AG Gonz\'alez of the Institute of Theoretical Chemistry of the University of Vienna with contributions from Jes\'us Gonz\'alez-V\'azquez, Matthias Ruckenbauer, Markus Oppel, Patrick J.\ Zobel, and Leticia Gonz\'alez.
...
\begin{itemize}
  \item A Fortran90 compiler (This release is tested against \link{https://gcc.gnu.org/fortran/}{GNU Fortran} \todo{4.4.7} and \link{https://software.intel.com/en-us/fortran-compilers}{Intel Fortran} \todo{15.0}).
  ...
  \item \link{https://www.python.org/downloads/release/python-278/}{Python 2} (This release is tested against Python 2.6.6 and 2.7.3).
\end{itemize}
...
\todo{Add example directory and update installation description.}
...
\begin{itemize}
  ...
  \item A program for molecular visualization, able to read files in the xyz file format (e.g.\ \link{http://www.cmbi.ru.nl/molden/molden.html}{\textsc{Molden}}, \link{http://gabedit.sourceforge.net/}{\textsc{Gabedit}}, \link{http://molekel.cscs.ch/wiki/pmwiki.php}{\textsc{Molekel}} or \link{http://www.ks.uiuc.edu/Research/vmd/}{VMD})
  ...
  \item The \link{http://orcaforum.cec.mpg.de}{\textsc{Orca}} ab initio package.
\end{itemize}
...
\begin{itemize}
  \item \link{http://www.molpro.net/}{\textsc{Molpro}} (this release was checked against \textsc{Molpro} 2010 and 2012 \todo{2015?}).
  \item \link{http://http://molcas.org/}{\textsc{Molcas}} (this release was checked against \textsc{Molcas} 7.8 and 8.0).
  \begin{itemize}
    \item \link{http://dasher.wustl.edu/tinker/}{\textsc{Tinker}}, interfaced to \textsc{Molcas} 7.8 or 8.0 for QM/MM dynamics.
  \end{itemize}
  ...
  \item \link{http://www.turbomole.com}{\textsc{Turbomole}} (this release was checked against \textsc{Turbomole} 6.6 and 7.0; \todo{version 7.1 is currently not supported}).
  \begin{itemize}
    \item \link{http://orcaforum.cec.mpg.de}{\textsc{Orca}} for spin-orbit couplings.
  \end{itemize}
...
Remember that \ttt{molcas\_input.py} cannot create multi-job templates or templates with the \ttt{basis\_external} keyword.
...
A functioning force field file which exemplifies the format is shown in \ttt{\$SHARC/../examples/SHARC\_ADF/ADF.qmmm.ff}.
Here, we only show briefly the format:
\begin{example}
  \begin{verbatim}
...
An example of the file is given in \ttt{\$SHARC/../examples/SHARC\_ADF/ADF.qmmm.table}.
...
\end{itemize}
\todo{Please cite these references (add to chapter 2!)} if you use the the \ttt{wfoverlap.x} code inside \sharc\ for these purposes.
...
If you saved the restart files in \textsc{Amber}'s newer NetCDF format, \ttt{cpptraj} can also be used to convert them to ASCII restart files.
If you did not save restart files, \ttt{cpptraj} can even be used to generate restart files from the trajectory file (\ttt{.mdcrd}).
