...
# ======================================================================= #
def getcienergy(out,mult,state,version,method):
    ...
    if 'h' in QMin:
        ...
        for istate,i in enumerate(QMin['statemap'].values()):
            ...
            if states[mult-1]==1 and method==2:
                ...
                method1=method
            h[istate][istate]=complex(getcienergy(out,mult,state,version,method1))
        QMout['h']=h
...
def generate_joblist(QMin):
    ...
    if QMin['gradmode']==0:
        ...
        QMin2=deepcopy(QMin)
        remove=['soc','dm','always_guess','always_orb_init','comment','ncpu','init','veloc','overlap']
        for r in remove:
...
# ======================================================================= #
def collectOutputs(joblist,QMin):
...
def numdiff(enp,enn,enc,displ,o1p,o2p,o1n,o2n,iatom,idir):
    ...
    if o1p==0.0 or o2p==0.0:
        if DEBUG:
            print 'Using one-sided NumDiff for iatom=%i, idir=%i. Using only negative displacement.' % (iatom,idir)
        g=(enc-enn)/displ
    elif o1n==0.0 or o2n==0.0:
        if DEBUG:
            print 'Using one-sided NumDiff for iatom=%i, idir=%i. Using only positive displacement.' % (iatom,idir)
        g=(enp-enc)/displ
    else:
        g=(enp-enn)/2./displ
    return -g
            ...
            for iatom in range(QMin['natom']):
                for xyz in range(3):
                    ...
                    for istate in range(QMin['nmstates']):
                        ...
                        g=numdiff(enp,enn,0.0,displ,1.0,1.0,1.0,1.0,iatom,xyz)
...
def main():
    ...
    # run all MOLCAS jobs
    runjobs(joblist,QMin)

    # get output
    QMoutall=collectOutputs(joblist,QMin)
