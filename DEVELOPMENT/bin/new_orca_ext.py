...
def close_keystrokes():
  KEYSTROKES.close()
  shutil.move('KEYSTROKES.tmp','KEYSTROKES.setup_orca')
...
#        print 'File does not contain initial conditions!'
 ...
 # INFOS['initf']=initf
  ...
  while True:
    rmult,rstate=tuple(question('Root:',int,[0, 0]))
    if rmult == 0:
      print 'Only one state choosen, performing an optimization'
      INFOS['cas.root2'] = ''
      INFOS['calc_ci']='opt'
      break
    else:
      ...
      print INFOS['cas.root1'], INFOS['cas.root2']
      ...
      if INFOS['cas.root1'][0]==INFOS['cas.root2'][0]:
        ...
        INFOS['calc_ci']='cross'
      summe = 0
      for i in range(INFOS['cas.root2'][0]-1):
        summe += states[i]*(i+1)
      summe += INFOS['cas.root2'][1]
      print summe
      INFOS['cas.root2']=summe    
      break

  summe = 0
  for i in range(INFOS['cas.root1'][0]-1):
    summe += states[i]*(i+1)
  summe += INFOS['cas.root1'][1]
  print summe
  INFOS['cas.root1']=summe
  crossing = str(INFOS['cas.root1']) + ' ' + str(INFOS['cas.root2'])



#  print centerstring('Optimization method',60,'-')
#  print '\nPlease enter the two states that make the crossing\ne.g. 1 2 for the crossing between the ground state and the first excited state.'
#  while True:
#    crossing=question('Crossing between: ',int)
#    if len(crossing)==0:
#      continue
#    if any(i<0 for i in crossing):
#      print 'There are no negative states!'
#      continue
#    break
  print ''
  print 'Chosen states: '+str(crossing)
  INFOS['crossing']=crossing

  if INFOS['calc_ci'] == 'ci':
    print centerstring('Optimization parameter',60,'-')
    print '\nPlease enter the values for the alpha and beta parameters.'
    while True:
      alpha=question('Alpha: ',float)
      break
    while True:
      beta=question('Beta: ',float)
      break
    INFOS['alpha']=alpha
    INFOS['beta']=beta
    ...
    while True:
      maxstep=question('maxstep: ',float)
      break
    while True:
      trust=question('trust: ',float)
      break
    INFOS['maxstep']=maxstep
    INFOS['trust']=trust   
...
#'''
  print 'Do you want to calculate here (%s)\n' % (INFOS['cwd'])
...
cp $COPY_DIR/* $PRIMARY_DIR
rm -r $COPY_DIR
''' % (projname,INFOS['cwd'], INFOS['copydir'])
...
''' % (INFOS['nstates'],INFOS['calc_ci'],INFOS['crossing'],Interfaces[INFOS['interface']]['name'],INFOS['alpha'][0],INFOS['beta'][0],INFOS['maxstep'][0],INFOS['trust'][0],INFOS['geom_location'])
  else:
    string='''#SHARC: states %s
#SHARC: opt %s %s 
#SHARC: interface %s
